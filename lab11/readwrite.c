...
/* MAIN FUNCTION. Open 'students.dat' file, read data, print data and write it
 * to binary file.
 */
int main()
    {
    FILE *pStudentsInput;                       /* File pointer to 'students.dat', if it open */
    FILE *pNewStudents;                         /* File pointer to 'newstudents.dat', if it open */
    STUDENT_T students[MAXCOUNT];   /* Declare structure of array to get data */
    int i = 0;                                           /* Used for count loop */
    int status = 0;                                   /* Check status when close file */
    /* Open 'students.dat' for read binary file */
    openFile("students.dat", 1, &pStudentsInput);
    ...
    closeFile(&pStudentsInput, "students.dat");  /* Close 'students.dat' file */
    /* Open 'newstudents.dat' for write binary file */
    openFile("newstudents.dat", 2, &pNewStudents);
    /* Write output binary file */
    fwrite(students, sizeof(STUDENT_T), i, pNewStudents);
    closeFile(&pStudentsInput, "newstudents.dat");  /* Close 'newstudents.dat' file */
    printf("Output file: newstudents.dat\n");
    printf("Good bye!\n");
    exit(0);
    }
